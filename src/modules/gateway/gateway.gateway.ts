	...
	implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect
{
	...
	handleDisconnect(client: AuthenticatedSocket): void {
		...
		client.broadcast
			.to(client.roomId)
			.emit(SOCKET_EVENTS.SIGNOUT, client.clientId);
	...
	// ----------------------------------Event listeners-------------------------------
	@SubscribeMessage(SOCKET_EVENTS.TRANSFER_REQUEST_SEND_FILE)
	handleRequestTransfer(
		...
		if (!data.receivedClientId) {
			return client.emit('error', new Error('The user not found!'));
		}
		...
		if (!receivedSocket) {
			return client.emit('error', new Error('The device not found!'));
		}
		...
		ioServer
			...
			.emit(SOCKET_EVENTS.TRANSFER_WAIT_TRANSFER_ACCEPTED, client.user.email);
	}

	@SubscribeMessage(SOCKET_EVENTS.TRANSFER_SEND_FILE)
	handleSendFile(
		@ConnectedSocket() client: AuthenticatedSocket,
		@MessageBody()
		data: {
		...
		client.broadcast
			...
			.emit(SOCKET_EVENTS.TRANSFER_RECEIVE_FILE, data.file);
	}

	@SubscribeMessage(SOCKET_EVENTS.TRANSFER_REPLY_TO_REQUEST)
	handleResponse(
		@ConnectedSocket() client: AuthenticatedSocket,
		@MessageBody()
		...
		if (data.confirm) {
			client.broadcast
				.to(client.transferRoom)
				.emit(SOCKET_EVENTS.TRANSFER_ACCEPT_REQUEST);
		} else {
			client.broadcast
				.to(client.transferRoom)
				.emit(SOCKET_EVENTS.TRANSFER_REFUSE_REQUEST);
			ioServer.in(client.transferRoom).socketsLeave(client.transferRoom);
		}
	}

	@SubscribeMessage(SOCKET_EVENTS.TRANSFER_ACK_RECEIVE_FILE)
	handleAcknowledge(
		@ConnectedSocket() client: AuthenticatedSocket,
		@MessageBody()
		...
		client.broadcast
			.to(client.transferRoom)
			.emit(SOCKET_EVENTS.TRANSFER_ON_ACK_RECEIVE_FILE, data.ack);
	...
	@SubscribeMessage(SOCKET_EVENTS.TRANSFER_CANCEL_TRANSFER)
	handleCancelTransfer(@ConnectedSocket() client: AuthenticatedSocket) {
		client.broadcast
			.to(client.transferRoom)
			.emit(SOCKET_EVENTS.TRANSFER_ON_CANCEL_TRANSFER);
	}
}
